...
# # # # # # # # # # # # # # # # # # # # # # 	
	...
	the_y <- "bilob_ratio_gt1"
	phyto <- phyto[which(phyto$bilobate_ratio > 0),]
	phyto[, the_y] <- phyto$bilobate_ratio
	log_adj <- 0
	
	the_y <- "bilob_soil_gt1"
	phyto <- phyto[which(phyto$bilobate_ratio > 0),]
	phyto[, the_y] <- phyto$bilobates_per1000gsoil
	log_adj <- 0
	
	hist(phyto[,the_y])
	hist(log(phyto[,the_y]))
...
# GAM
	...
	predictors[["pcClim1"]] <- predictors[["pcSoil1"]] <- predictors[[1]] 
	predictors[[paste0('pcClim',1)]][] <- predict(pcgam.clim,predictors[])[,1]
	predictors[[paste0('pcSoil',1)]][] <- predict(pcgam.soil,predictors[])[,1]

	PRG <- aggP[[ c(paste0('pcClim',1:3), paste0('pcSoil',1:3), 'dcoast')]]
	
#	f1.gam <- I(log(phyto[,the_y] + log_adj )) ~  s(dcoast) + s(bio1) + s(bio7) + s(bio12)  + s(bio15) + s(ph) + s(bulk) + s(sand) + s(wetland) 
	f1.gam <- I(log(phyto[,the_y] + log_adj )) ~  s(dcoast) + s(pcClim1)  + s(pcSoil1)  
...
ens.stat <- c( 'cor' = cor( p.ens, obs), 
	...
	'I' = round(moran( p.ens - obs, phyto[, c('lon','lat')] )$p.value,4)
	)
